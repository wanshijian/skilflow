...
- **去重关键**：识别为标题的文本**必须从正文中删除**，不要让它同时出现在 sections 的 heading 和紧随其后的 paragraph 中。标题独立成 heading，正文从此标题之后的文本开始。
...
8. **落款格式**：发文机关署名和成文日期置于正文右下方。署名位于距正文一行位置，日期位于署名正下方。成文日期用阿拉伯数字将年、月、日标全（如"2025年12月31日"），年份用全称，月、日不编虚位。成文日期右空四字，发文机关署名在成文日期之上居中。
9. **字体缺失处理**：当方正小标宋简体不可用时→宋体二号；仿宋GB-2312 不可用时→仿宋三号；楷体GB-2312 不可用时→楷体三号。
...
| 标题与正文重复 | 标题提取为 heading 后**必须从正文段落中删除**，不可在首个 paragraph 中残留标题文本 |
