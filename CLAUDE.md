...
触发词：「整理」「清洗」「排版」「格式化」、format 参数

→ 加载 `doc-cleanup` skill，按其中规范执行 6 步流程：

```
符号清洗 → 结构识别 → 段落重组 → 排版规范 → 标点统一 → 质量校验
```

输出：严格 JSON（无 markdown 包裹），格式和字段定义详见 skill 文件。

核心原则：不是删符号，是重构——把散乱文本变成**层次分明、段落清晰、阅读流畅**的专业文档。
